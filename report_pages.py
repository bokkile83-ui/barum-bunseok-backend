...
#   [원인] 4쪽 HEART4가 <b>세 담보를 모두 「허혈성 진단비」 한 칸에서</b> 읽고 있었다.
#     세 줄 모두 허혈성 행을 키로 삼았고, 협심증은 점표기(dot-only) 접두까지 붙어 있었다.
#   ⇒ 허혈성 하나의 값이 <b>세 줄에 그대로 복제</b>돼 「2배수」로 보였고, 엑셀·보장분석지와 어긋났다.
#     반대로 <b>허혈성이 0이면 협심증·급성심근경색이 영구 빈칸</b>이 된다(등식2 위반).
#   ⇒ <b>각 담보는 자기 마스터 행만 읽는다.</b> 빈맥도 마스터에 행이 있는데 None이라 늘 빈칸이었다.
HEART4 = [
...
#   다르면 발행을 막는 검사 · 심장 전용으로만」)
#   [배경] 4쪽이 남의 행(허혈성)을 읽어도 조문검사는 <b>38/38 PASS</b>였다. 조문검사는
#     「문자열이 있나」만 본다 — <b>값이 같은지는 값으로 검사해야 한다</b>(조용히 틀림 → 시끄럽게 틀림).
#   ★이 표는 <b>마스터 심장 행 이름</b>에서 온다. HEART4를 참조하지 않는다 —
#     참조하면 자기 자신을 검사하는 꼴이라 아무것도 못 잡는다.
HEART_SYNC = [('급성심근경색', '급성심근경색'), ('협심증', '협심증'),
              ('허혈성', '허혈성 진단비'), ('심장판막', '심장판막'),
              ('심근 · 심내막 염증', '염증'), ('빈맥', '빈맥'),
              ('부정맥', '부정맥'), ('심부전', '심부전'), ('심근병증', '심근병증')]
...
def heart_audit(cmp_, client='고객', base_date=''):
    """★제121조 — 진단서 4·5쪽에 실제로 찍힌 심장 값이 엑셀(cov)과 같은지 대조한다.
       다르면 그 목록을 돌려준다(빈 목록 = 통과)."""
    cov = cmp_['new']['cov']
    bad = []
    for _pg, _fn in ((4, p4), (5, p5)):
        try:
            html = _fn(cmp_, client, base_date)
        except Exception as e:
            bad.append('%d쪽 렌더 실패: %s' % (_pg, e)); continue
        for lb, key in HEART_SYNC:
            want = int(cov.get(key, 0) or 0)
            got = _hv(html, lb)
            if got is None:
                continue                      # 그 쪽에 칸이 없는 담보는 검사 대상 아님
            if got != want:
                bad.append('%d쪽 %s — 엑셀 %s / 진단서 %s' % (_pg, lb, format(want, ','), format(got, ',')))
    return bad
...
def p5(cmp_, client='고객', base_date='', pg=5, totpg=7):
    ...
    def g(k):
        # ★v588 — `__TWO2__`는 동명 2행(뇌·심)을 max로 접은 값이다(아래 `_rows2`).
        if k == '__TWO2__':
            return _TWO2[0]
        return cov.get(k, 0)
    ...
    two, circ = (max(_rows2) if _rows2 else cov.get('2대 주요치료비', 0)), 0
    _TWO2[0] = two          # ★v588 표·카드가 같은 값을 본다
    if two:
        circ, two = two, 0
    ...
    right = ('<table style="width:100%;border-collapse:collapse">' + row('뇌혈관', g('뇌혈관진단비'), st=True)
             ...
             + ''.join(row(_lb, g(_k), ind=True)
                       for _lb, _k in (('협심증', '협심증'),
                                       ...
                                       ('빈맥', '빈맥'),
                                       ('산정특례 심장', '산정특례심장'),
                                       # ★★★★★v588 (지점장 실측 2026.08.25 「리포트에 2대주요치료비가
                                       #   <b>위에는 2천 아래는 1천</b>이다. <b>정답은 1천</b>이다」 · 천미옥).
                                       #   [원인] 같은 페이지에 경로가 둘이었다 —
                                       #     아래 카드는 `_rows2`의 <b>max(1,000)</b>를 쓰는데(878행),
                                       #     이 표만 `g()`로 <b>cov dict</b>를 봤다. cov는 담보명이 키라
                                       #     <b>뇌 1,000 + 심장 1,000이 합산돼 2,000</b>이 됐다.
                                       #   ⇒ 이미 max로 구해 둔 값을 <b>이 표도 그대로</b> 쓴다(경로 단일화).
                                       ('2대 주요치료비', '__TWO2__'),
    ...
    return ('<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><style>%s</style></head><body>'
            ...
            '<p>V191 · 수술·약제 또는 뇌출혈 급성기 입원·뇌경색 NIHSS 5↑ · 30일</p>'
            '<div class="small-line"><div class="st">%s</div>'
            '<div class="vb"><div class="value-box">%s</div></div>'
            '<div class="un"><span class="unit">만원</span></div></div></div>'
            '<div class="special-card"><h4>산정특례 (심장)</h4>'
            '<p>V192 · 수술·시술 또는 고시약제 입원 필수 · 30일(이식 60일)</p>'
            '<div class="small-line"><div class="st">%s</div>'
            '<div class="vb"><div class="value-box">%s</div></div>'
            '<div class="un"><span class="unit">만원</span></div></div></div>'
            '</section></main>'

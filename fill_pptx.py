"""fill_pptx.py v2 — 보장분석 JSON → 보장분석지 PPT 폼(그룹 중첩) 전수 채움.
박스 위치 보존(텍스트만 교체) → 27/32조 자동. 28조: 버블 word_wrap=False.
박스별 라벨맵으로 그룹 내부까지 재귀 채움. 매핑 안 된 칸은 빈칸 유지(안전)."""
import re
...
BUBBLES = {
    (3.6, 4.8): ["뇌혈관진단"], (3.7, 6.3): ["뇌졸"],
    (3.7, 7.5): ["뇌출혈"], (13.5, 7.5): ["급성심근", "허혈성"],
}
LABELBOX = {
    (14.4, 15.2): [(["질병 3%", "질병3%"], ["질병후유3"]), (["질병 80%"], ["질병후유80"]),
                   (["상해3%", "상해 3%"], ["상해후유3"]), (["상해 80%"], ["상해후유80"]),
                   (["교통후유"], ["교통후유"])],
    (14.4, 9.5): [(["대인"], ["벌금대인", "대인"]), (["대물"], ["대물"]), (["합의금"], ["처리지원", "합의"]),
                  (["6주"], ["6주"]), (["변호사"], ["변호사"]), (["자부상"], ["부상위로", "자부상"])],
    (14.4, 19.5): [(["치매"], ["치매", "장기요양"]), (["재가"], ["재가"]),
                   (["노치원"], ["노인전문", "노치원"]), (["시설"], ["시설급여"])],
    (14.4, 24.1): [(["크라운"], ["크라운"]), (["임플란트"], ["임플란트"]),
                   (["기타"], ["치아기타"]), (["화재"], ["화재"])],
    (9.9, 4.3): [(["협심증", "심부전"], ["협심증", "심부전"]), (["부정맥"], ["부정맥"])],
    (1.9, 2.3): [(["산정특례"], ["산정특례뇌", "산정특례(뇌"]), (["혈전용해"], ["혈전용해치료비(뇌", "혈전용해뇌"]),
                 (["2대주요"], ["2대주요치료비(뇌", "2대주요뇌"])],
    (11.0, 2.2): [(["산정특례"], ["산정특례심", "산정특례(심"]), (["혈전용해"], ["혈전용해치료비(심", "혈전용해심"]),
                  (["2대주요"], ["2대주요치료비(심", "2대주요심"])],
    (0.6, 13.9): [(["암주요"], ["암주요치료비"])],
    (9.8, 13.2): [(["일상", "배상"], ["일상생활배상"])],
    (9.9, 15.6): [(["입원"], ["실손입원"]), (["통원"], ["실손통원"]), (["MRI"], ["MRI"]),
                  (["도수"], ["도수"]), (["비급여주사"], ["비급여주사"])],
    (9.9, 9.4): [(["5대골절"], ["5대골절진단비"]), (["골절"], ["골절진단비"]), (["중대화상"], ["중대화상진단비"]),
                 (["화상"], ["화상진단비"]), (["반깁스"], ["반깁스"]), (["깁스"], ["깁스치료"]),
                 (["응급실"], ["응급실"])],
    (9.8, 19.5): [(["___세", "- _", "－세"], ["질병사망(__"]), (["80세"], ["질병사망(80"]),
                  (["종신"], ["일반사망(종신", "일반사망"])],
    (9.8, 21.8): [(["상해사망"], ["상해사망"])],
    (0.5, 9.3): [(["암진단비"], ["암진단비"]), (["유사암"], ["유사암"]), (["통합암"], ["3대암진단비"]),
                 (["전이암"], ["전이암진단비"]), (["항암치료"], ["항암치료비"]), (["표적"], ["표적항암"]),
                 (["양성자"], ["양성자"]), (["세기"], ["세기조절"]), (["다빈치"], ["다빈치"]),
                 (["하이클래스"], ["하이클래스"]), (["암통원"], ["암통원"])],
    (0.3, 15.7): [(["질병수술"], ["질병수술비"]), (["종합병원수술"], ["종합병원수술비(질"]),
                  (["119대"], ["119대"]), (["뇌혈관수술"], ["뇌혈관수술"]), (["허혈성수술", "심장수술"], ["심장수술"])],
    (4.8, 15.8): [(["상해수술"], ["상해수술비"]), (["종합병원수술"], ["종합병원수술비(상"]),
                  (["5대골절수술"], ["5대골절수술"]), (["골절수술"], ["골절수술"]), (["화상수술"], ["화상수술"]),
                  (["중대상해수술"], ["중대상해수술"]), (["창상봉합"], ["창상봉합"])],
    (0.3, 21.9): [(["질병일당"], ["질병일당"]), (["상해일당"], ["상해일당"]), (["중환자실"], ["중환자실"]),
                  (["상급병원일당", "1인실 상급"], ["1인실상급"]), (["종합병원일당", "1인실 종합"], ["1인실종합"]),
                  (["암일당"], ["암일당입원", "암일당"]), (["간병인"], ["간병인사용"]),
                  (["요양병원"], ["요양병원"]), (["간호통합"], ["간호통합"])],
}
...
def _bubble(sh, text, renew):
    ...
    for r in p.runs:
        ...
        r.font.color.rgb = RGBColor(0x00, 0x00, 0xFF) if renew else RGBColor(0x22, 0x22, 0x22)

def _labelfill(sh, mapping, cov):
    tf = sh.text_frame
    acc = {}  # entry_idx -> [sum, renew]
    for rw in cov:
        nm = _n(rw.get("name")); best = -1; blen = 0
        for i, (keys, subs) in enumerate(mapping):
            for x in subs:
                xn = _n(x)
                if xn and xn in nm and len(xn) > blen:
                    best, blen = i, len(xn)
        if best >= 0:
    ...
    for ln in tf.text.split("\n"):
        if ":" in ln:
            label = ln.split(":")[0]; val = ""
            for i, (keys, subs) in enumerate(mapping):
                if any(k in label for k in keys):
                    s = acc.get(i)
                    val = won(s[0]) if s and s[0] > 0 else "미가입"
                    break
            out.append(label.rstrip() + " : " + val)
        else:
            out.append(ln)
    tf.text = "\n".join(out)
...
def fill_pptx(data, xlsx_path, form_path, out_path):
    ...
    for sh in p.slides[0].shapes:
        if sh.has_text_frame and "기준" in sh.text_frame.text:
            sh.text_frame.text = f"{date.today():%Y년 %m월 %d일} 기준"
    for sh in _flatten(p.slides[0].shapes):
        if sh.has_text_frame and "보장" in sh.text_frame.text and "님" in sh.text_frame.text:
            sh.text_frame.text = f"{client}님의 보장"
    memo = data.get("memo", [])
    body = f"보장분석 메모 — 값 확인 필요 ({client})\n" + ("\n".join(f"● {m}" for m in memo) if memo else "● 특이사항 없음")
    p.slides[1].shapes[0].text_frame.text = body
    p.save(out_path)
    return out_path

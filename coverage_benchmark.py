# ===== BARUM coverage_benchmark.py v750-jongdesign-20260918 (구 v33-ci-rate-20260708 계승) =====
# -*- coding: utf-8 -*-
"""
...
def map_excel_to_report(xlsx_path, settings=None, age_band='40s', age_known=False):
    ...
    # ★★★★★v608 (지점장 실측 2026.08.30 「뇌혈관만 가입되어 있는데 진단서에 뇌졸증이 찍히고
    #   급성심근경색이 없는데 가입되어 있다고 찍힌다. <b>엑셀=보장분석ppt=진단서를 일치화</b>하라」).
    #   [원인] v270이 <b>뇌출혈 줄만</b> 고쳤고 아래 두 줄에는 그 원칙이 안 들어갔다.
    #     뇌졸증 칸이 <b>뇌혈관진단비</b>를, 급성심근 칸이 <b>허혈성 진단비</b>를 복사했다.
    #     실측 정상범 — 엑셀 뇌졸증 빈칸·급성심근 빈칸인데 진단서에 값이 찍혔다(등식1 위반).
    #   ⇒ <b>각 칸은 자기 축만 본다.</b> 없으면 '미가입'. (v270과 같은 원칙 · 세 줄 모두 적용)
    _p5=[('뇌출혈진단비',_gv('뇌출혈진단비') or _gv('중대한 뇌출혈')),
         ('뇌졸증진단비',_gv('뇌졸증진단비') or _gv('중대한 뇌졸증')),
         ('급성심근경색',_gv('급성심근경색') or _gv('중대한 급성심근')),
         ('뇌혈관진단비',_gv('뇌혈관진단비')),

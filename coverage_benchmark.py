# ===== BARUM coverage_benchmark.py v368-simjang-20260806 (구 v33-ci-rate-20260708 계승) =====
# -*- coding: utf-8 -*-
"""
...
def map_excel_to_report(xlsx_path, settings=None, age_band='40s', age_known=False):
    ...
    _p5=[('뇌출혈진단비',_gv('뇌출혈진단비') or _gv('중대한 뇌출혈')),
         ...
         # ★★★★★v363 영구지침(지점장 지적 2026.08.06 "ci 급성심근경색만 있는데 일반 급성심근경색에도
         #   계속 찍힌다"): <b>급성심근경색 칸은 급성심근 축(급성심근경색 행)만 본다.</b>
         #   구 코드는 `or _gv('중대한 급성심근')` 폴백이 있어, 일반 급성심근 담보가 <b>하나도 없는</b>
         #   CI 고객에게 <b>CI 본체 값을 일반 칸에 복사</b>했다.
         #   [실측 이명순 진단서] 9p 전체담보표 급성심근경색 = 빈칸(엑셀 행 0) 인데
         #   7p 워크시트 급성심근경색 = <b>4,000만</b>(=중대한 급성심근 본체) → <b>같은 담보가 두 산출물에서
         #   다르다 = 결과값 동결 위반</b>.
         #   ★v270이 뇌출혈 칸에 대해 이미 고친 것과 <b>동일한 결함</b>이 급성심근 줄에만 남아 있었다.
         #   ★`_gv('허혈성 진단비')` 폴백도 같은 구조이나 지점장 지시 범위 밖이므로 <b>그대로 둔다</b>([확인]큐).
         ('급성심근경색',_gv('급성심근경색') or _gv('허혈성 진단비')),
         ('뇌혈관진단비',_gv('뇌혈관진단비')),

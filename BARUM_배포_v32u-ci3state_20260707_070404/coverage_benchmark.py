# -*- coding: utf-8 -*-
# ★v32u-ci3state-20260707: CI 3상태 판정(회색지대 [확인]) 추가
"""
...
def map_excel_to_report(xlsx_path, settings=None, age_band='40s', age_known=False):
    ...
    _ci_present=bool(_ci_pairs or _ci_apply>0)
    # ★v32u 3상태 판정(ci/check/none): 상품명(CI/GI/리빙케어)까지 반영해
    #   '담보값만 보던 2상태'가 회색지대(상품명 CI인데 중대한OO 미추출)를 비CI로 오분류하던 버그 차단.
    _CI_KW=('CI보험','GI보험','리빙케어','CI 보험','GI 보험')
    _ci_prods=[h['nm'] for h in headers if any(k in (h['nm'] or '') for k in _CI_KW)]
    if _ci_present:
        _ci_status='ci'          # 중대한OO 담보값 실제 매핑 → CI 선지급 분석
    elif _ci_prods:
        _ci_status='check'       # 상품명은 CI인데 담보값 미추출 → [확인](비CI 낙하 금지)
    else:
        _ci_status='none'        # CI 표기 없음 → 비CI(Plan B)
    ci={'present':_ci_present,'status':_ci_status,
        'product':(_ci_prods[0] if _ci_prods else ''),'products':_ci_prods,
        'samang':_fmt(_ci_samang),
    ...
    noci={'present':(_ci_status=='none') and bool(noci_items),'items':noci_items}

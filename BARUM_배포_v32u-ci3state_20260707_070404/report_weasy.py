# -*- coding: utf-8 -*-
# ★v32u-ci3state-20260707: P3 CI블록 3상태 분기(ci/check/none) 추가
...
<td class="bv" style="color:{(BLUE if c["renew"] else NAVY)}">{c["amt"]:,}</td></tr>'''
    ...
    _cistat=ci.get('status') or ('ci' if ci.get('present') else 'none')  # ★v32u 3상태(ci/check/none)
    if _cistat=='ci':
    ...
    # ── 회색지대([확인]) / 비CI(Plan B) 분기 (CI 미보유 시 P3 상단 대체) ──
    noci=rep.get('noci',{'present':False})
    if _cistat=='check':
        # 회색지대: 상품명은 CI/GI/리빙케어인데 중대한OO 담보값 미추출 → 판정 보류·수기 확인
        _prd=_html.escape(ci.get('product','') or '해당 계약')
        noci_html=('<div class="sect">핵심 보장 분석 <span>CRITICAL ILLNESS · 판정 보류 [확인]</span></div>'
                   '<div class="noci-wrap">'
                   '<div class="noci-big" style="color:#B8860B">CI 판정 보류 [확인]</div>'
                   '<div class="noci-one" style="border-left-color:#B8860B;background:#FBF7EC">'
                   '<div class="t" style="color:#8A6D1B">■ 회색지대 — 수기 확인 필요</div>'
                   f'<div class="d"><b>{_prd}</b>은(는) 상품명에 <b>CI·GI·리빙케어</b> 표기가 있으나, 자동 추출에서 <b>중대한(암·뇌졸증·급성심근) 담보값</b>이 확인되지 않았습니다. 선지급률(50%/80%) 판정을 <b>보류</b>합니다. 별첨 약관에서 <b>주계약 사망보험금·선지급 비율·잔여분</b>을 직접 확인해 CI 선지급형/일반형 여부를 확정하십시오.</div>'
                   '</div>'
                   '</div>')
    elif _cistat=='none':

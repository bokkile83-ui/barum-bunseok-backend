...
def _donut(pct, color, size=120, sw=13):
...
<text x="{c}" y="{c}" text-anchor="middle" dy="0.36em" font-size="28" font-weight="700" fill="{NAVY}" font-family="NanumSquareRound">{pct}%</text>
</svg>'''
...
.rbox .rh {{ padding:3mm 4mm; color:#fff; font-weight:700; font-size:10.5pt; }}
.rbox.b .rh {{ background:{BLUE}; }} .rbox.k .rh {{ background:#2A3340; }}
.rbox .rh small {{ float:right; font-weight:400; font-size:8.5pt; }}
.rbox .pr {{ padding:1.5mm 4mm; font-size:9.5pt; overflow:hidden; }}
...
.dcell {{ text-align:center; padding:4mm 0; }}
.dcell .dn {{ font-size:10pt; font-weight:700; color:{NAVY}; margin-top:1mm; }}
.legend {{ display:flex; justify-content:center; gap:8mm; margin:3mm 0 1mm; font-size:9pt; color:{INK}; }}
.legend span {{ display:flex; align-items:center; gap:2mm; }}
.legend i {{ width:4mm; height:4mm; border-radius:1mm; display:inline-block; }}
.sect2 {{ font-size:12pt; font-weight:800; color:{NAVY}; margin:6mm 0 2mm; border-bottom:1.5pt solid {GOLD}; padding-bottom:1.5mm; }}
...
.btab td {{ padding:1.8mm 3mm; text-align:center; border-bottom:.5pt solid {LINE}; color:{INK}; }}
...
<div class="pg">
 ...
 <div class="body">
  ...
  <div class="sect" style="margin-top:6mm">갱신 / 비갱신 구조 <span>RENEWAL</span></div>
  <table class="ren"><tr>
  ...
  <div class="sect" style="margin-top:6mm">월 보험료 구성 <span>PREMIUM</span></div>
  <table class="pbar">{bars}</table>
  <div class="note"><b>분석 기준</b> — 본 진단서는 보유 {n_contract}개 계약의 별첨 보장현황을 전수 집계한 결과다. 금액은 담보별 가로 합계(만원), 보험료는 월납 기준이며, 권장 가입액은 상담을 통해 별도 산정한다.</div>
 </div>

...
def _valueset(rep):
    ...
    for _v in rep.get('_p8vals', []):
        add(_v)
    # ★★★★★v417 (지점장 지적 2026.08.13 「7페이지는 제안서 넣어도 다 블랙or블루다」):
    #   7p 진단비 행의 <b>보유/제안 분리 숫자</b>가 이 집합에 없어 편집칸이 안 생겼고,
        ...
        for ln in pg.findall('.//x:line', NS):
            ...
            for w in ws[1:]:
                ...
                if gap <= (cur[4] - cur[2]) * 0.6:          # 글자 높이의 60% 안이면 같은 덩어리
                    # ★★★★★v619 (지점장 실측 2026.08.30 · 정답지 대조 — 정답 「I63 · 65 · 66」,
                    #   내 것 「I63·65·66」 · 「1,000 + 1,000」→「1,000 +1,000」).
                    #   [원인] 공백 판정 기준이 <b>글자 높이의 18%</b>였다. 「·」 앞뒤 간격은
                    #     그보다 좁아 <b>공백이 통째로 사라졌다</b>(19곳 실측).
                    #   ⇒ 기준을 <b>10%</b>로 낮춘다. 정답지 표기와 같아진다.
                    cur[0] += (' ' if gap > (cur[4] - cur[2]) * 0.10 else '') + w[0]

...
def build_report_pptx(rep, out, dpi=150):
    ...
    bs = " · ".join(f"{k} {v}" for k, v in (("뇌혈관", p5.get('뇌혈관진단비')), ("뇌졸증", p5.get('뇌졸증진단비')),
                                            ("허혈성", p5.get('허혈성 진단비')), ("급성심근", p5.get('급성심근경색')))
                    if v and v != '미가입') or "미가입"
    cards = [(0.42, "암 진단비", "걸렸을 때 일시금 (기본)", ci_items.get('ci암진단비') or ch.get('암주요치료비', '미가입')),

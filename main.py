...
DMAP = {
    ...
    '간병인사용질병입원일당(1일이상)(요양병원)(간편가입)(갱신형)':'간병인질병일당(요양병원)',   # ★v518
...
def resolve_kw(raw):
    ...
    if has('간병') and has('통합') and has('서비스') and (has('입원') or has('일당')) \
       and no('간병인','진단','수술','사망','치료비','후유','장해','보험료'):
        ...
        return '간호통합병동',0
    # ★★★★★v512 제110조 (지점장 지시 2026.08.19) — <b>요양병원 간병인을 기재한다</b>.
    #   구 조문 「요양병원 포함형 미기재」 <b>폐기</b>. 마스터에 <b>「간병인일당(요양병원)」 행 신설</b>(61행).
    #   지점장 원문 — 「보장분석지에서 간병인일당 / 간호통합병동 / 외에
    #   <b>간병인일당(요양병원)이라고 기재된 값 입력해줘</b>」
    #   ★'제외'가 붙은 것(요양병원제외)은 <b>일반 간병인</b> 행이다 — 아래 규칙으로 내려간다.
    # ★★★★★v518 제110조 2항 (지점장 확정 2026.08.19)
    #   「보장분석지에 <b>간병인질병일당(요양병원) / 간병인상해일당(요양병원)</b> — 이 값 입력하면 된다」
    #   ⇒ 요양병원 간병인은 <b>질병·상해 두 행</b>으로 나눈다(마스터 61·62행).
    if has('간병인') and has('요양병원') and no('제외'):
        if has('상해'): return '간병인상해일당(요양병원)',0
        return '간병인질병일당(요양병원)',0
...
def build_ppt(data, out, totals=None, surg_q=None, surg_s=None, splits=None):
    ...
    # ★v518 — 폼 칸은 하나이므로 질병·상해 중 <b>값이 있는 쪽</b>을 채운다(둘 다 있으면 질병 우선).
    if g('간병인질병일당(요양병원)'):   pvl('TextBox 22','요양병원 간병인','간병인질병일당(요양병원)')
    elif g('간병인상해일당(요양병원)'): pvl('TextBox 22','요양병원 간병인','간병인상해일당(요양병원)')

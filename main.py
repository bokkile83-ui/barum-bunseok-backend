...
def judge_renewal(product, expiry, pay_count, contract='', pay_period='', company='', dambo=None):
    ...
    if pay_y and cov_y:
        ...
        if pay_y >= 35 or ('삼성화재' in _co483):
            # ★★★★★v665 제153조 (지점장 확정 2026.09.04 장유진) — <b>전기납 판별 근거에서 「제도상 항상 갱신」 담보를 뺀다</b>.
            #   실측: 삼성화재 통합보험 수퍼플러스(1308) <b>80년납/2013~2093(80년)</b> = 전기납인데 갱신으로 나갔다.
            #   원인: `_is_gen_dambo`가 <b>일상배상책임은 갱신 표기가 없어도 True</b>(§6 「일상배상 무조건 갱신」)이고
            #   실손 특약도 갱신형 표기가 붙는다 — 이 둘은 <b>어느 계약에 있어도 갱신</b>이라 계약 성격을 말해주지 않는다.
            #   ⇒ 실손(입원·통원·약값)·일상배상책임·리셋월렛은 분자에서 제외. 그 밖의 담보에 갱신 표기가 있어야 계약 갱신.
            #   ★값 글자색(그 담보만 파랑)은 별개 층위 — 건드리지 않는다.
            _ALWAYS_GEN_ROWS = ('입원','통원','약값','약','일상배상책임','10억 플랜')
            def _informative(_k):
                _r = (resolve2(_k) or (None,))[0]
                return _r not in _ALWAYS_GEN_ROWS
            _has_gen = any(_is_gen_dambo(k, contract) for k in (dambo or {}).keys() if _informative(k))   # ★v476 제86조 + v665
...
_DEDUP_SELFTEST = [
    # ★v702 (지점장 확정 2026.09.15 「메리츠는 심장질환진단비 자체가 없다」): 정본표에 없는 담보 →
    #   임의 배정 금지. 기대값을 None(=[확인])으로 바꾼다. 구 '협심증/급성심근' 기대는 폐기.
    ('심장질환진단비Ⅰ',   None),
    ('심장질환진단비Ⅱ',   None),
    ('뇌질환진단비Ⅰ',     '뇌혈관진단비'),
    ('뇌질환진단비Ⅱ',     '뇌졸증진단비'),
...
_STRUCT_SELFTEST = [
    ...
    ('제157조 로마자등급',            'BARUM_DOCTRINE.md', r'로마자 Ⅰ·Ⅱ는 등급이 아닐 수 있다', True),
...
# ★★★★★v656 (지점장 지시 2026.09.03 「<b>메리츠 인식하자</b>」 · 박범수·박성희).
#   [문제] 메리츠 LIFE REPORT에는 KB식 한장보장표가 없어 <b>검산이 통째로 꺼졌다</b>.
#     `[v278 한장표] 표를 찾지 못했다 → 검산 생략` + `[v293 세부보충 차단] 보충 5건 폐기`.
#     그 5건에 <b>실손 입원·통원·종수술비</b>가 들어 있어 엑셀이 <b>0</b>이 됐다(실측 8건 불일치).
#   [해법] 메리츠 <b>「보장별 상세 내역」</b> 표를 한장표 대용 앵커로 쓴다(KB v295와 <b>같은 틀</b>).
#     구조 = `담보명  권장금액  <b>가입금액</b>  부족금액  과부족` → <b>두 번째 값이 가입금액</b>.
#   ★KB와 같은 이유로 <b>입원·통원·간병인·고액항암은 앵커에서 제외</b>(규칙 차이지 불일치가 아니다).
_MZ_SUM_MAP = {
    '질병사망':'질병사망(80세)', '상해사망':'상해사망',
    '질병 3%이상 후유장해':'질병후유3%', '상해 3%이상 후유장해':'상해후유3%',
    '일반암 진단비':'일반암', '유사암 진단비':'유사암(갑.기.경.제)',
    '통합암 진단비':'통합암',
    '뇌혈관 진단비':'뇌혈관진단비', '뇌졸중 진단비':'뇌졸증진단비',
    '뇌출혈 진단비':'뇌출혈진단비',
    '허혈성심장질환 진단비':'허혈성 진단비', '급성심근경색증 진단비':'급성심근경색',
    '암수술비':'암수술', '뇌혈관수술비':'뇌혈관수술비',
    '질병수술비':'질병수술비', '상해수술비':'상해수술비',
    '질병종수술':'질병 종수술비(1-5종)', '상해종수술':'상해 종수술비(1-5종)',
    '질병일당':'질병일당', '상해일당':'상해일당',
    '중입자방사선치료비':'중입자치료비',
    '교통사고처리 지원금':'합의금', '변호사 선임비용':'변호사',
    '벌금':'대인', '자동차사고 부상치료비':'자부상',
    '일상생활 배상책임':'일상배상책임', '화상 진단비':'화상진단비',
}

def parse_mz_summary(lines):
    """메리츠 '보장별 상세 내역' 표 → { 마스터담보명 : 가입금액(만원) }. 없으면 {}."""
    res = {}
    # ★<b>표 구간 안에서만 읽는다</b> — 1p 간편보장분석에도 같은 담보명이 나와
    #   전체를 훑으면 <b>권장금액이 가입금액을 덮는다</b>(실측 질병사망 0→1억).
    _idx = [i for i, l in enumerate(lines) if '보장별 상세 내역' in re.sub(r'\s+', ' ', l)]
    if not _idx:
        return res
    lines = lines[_idx[0]:]
    # ★가입금액이 <b>0</b>이면 토큰이 밀려 <b>부족금액을 읽는다</b>(실측 상해사망 18,900 · 질병사망 10,000).
    #   → <b>`0`과 음수(-)도 금액 토큰</b>으로 본다. 과부족 칸은 한글이라 자연히 멈춘다.
    # ★억 뒤 선택 그룹은 <b>「만」이 반드시 있어야</b> 한다 — `만?`로 두면
    #   `1억 0`을 한 덩어리로 삼켜 <b>다음 값이 밀린다</b>(실측 질병사망 0→1억).
    # ★메리츠는 <b>억과 만을 붙여 쓴다</b>(`1억8,900만`). 셀 구분은 <b>공백</b>이다.
    #   그래서 억 뒤 결합은 <b>공백 없이 붙은 경우만</b> 인정한다.
    #   안 그러면 `2억 1,100만`(권장 · 가입 — 다른 셀)을 한 덩어리로 삼켜 값이 밀린다.
    _AMT = r'(-?[\d,]+\s*억(?:[\d,]+\s*만)?|-?[\d,]+\s*만|0)'
    for l in lines:
        _l = re.sub(r'\s+', ' ', l.strip())
        if not _l: continue
        for _mz, _std in _MZ_SUM_MAP.items():
            # 담보명 뒤 <b>권장 → 가입</b> 순서 — 두 번째 값을 읽는다.
            m = re.search(re.escape(_mz) + r'(?![가-훃]) ' + _AMT + r' ' + _AMT + r'(?: |$)', _l)
            if not m: continue
            v = _kb_amt(re.sub(r'\s+', ' ', m.group(2)).strip())
            if v is None: continue
            res.setdefault(_std, v)          # ★표 안 첫 등장이 정본
            break
    if res: print(f'[v656 메리츠요약표] 검산 앵커 {len(res)}개 확보(한장표 대용)')
    return res
...
def _amt_kr(s):
    ...
    # ★★★★★v652 (지점장 지적 2026.09.03 「한화생명 사망보험금 잘못불러옴」 · 박성희).
    #   [실측] 원본 <b>「1,330만1천」</b>(=13,301,000원 · 1,330만원)을 <b>2,330</b>으로 읽었다.
    #   `만` 뒤의 「1천」은 <b>원 단위 끝자리</b>인데 구 파서가 <b>1,000만원으로 더했다</b>.
    #   「1억9,330만1천」도 20,330(정답 19,330). ⇒ <b>만이 있으면 천은 원단위로 보고 버린다</b>.
    #   「1천」처럼 <b>만 없이 천만</b> 있으면 종전대로 1,000만원이다(한장표 표기).
    _man, _cheon = m.group(2), m.group(3)
    v = int(m.group(1) or 0) * 10000 + int(_man or 0)
    if _cheon and not _man:
        v += int(_cheon) * 1000
...
def _sj_fixname(name, sj, comp, prod):
    ...
    # ★★★★★v654 (지점장 확정 2026.09.03 「Ⅱ 30만 + Ⅳ 20만 → <b>질병수술비 50</b>이 된다」 · 박성희).
    #   [원인] 삼성 담보명은 <b>`[건강]`이 앞에 붙는다</b> — `[건강]질병입원수술비Ⅱ(…)`.
    #   F3은 <b>문자열 시작</b>(`^`)을 보므로 대괄호 접두어 때문에 <b>안 걸렸다</b> → [확인]큐로 빠졌다.
    #   ⇒ <b>`[…]` 접두어를 떼고 판정</b>한다. 한장표(2p) 「질병수술비 (50만)」과 일치.
    _r3 = re.sub(r'^\[[^\]]*\]', '', r)
    if re.match(r'^질병입원수술비[ⅡⅢⅣⅤ]', _r3): return '질병입원수술비' + _r3[8:]
    # F6 생보 암/CI의 '뇌혈관진단' = 실제 뇌출혈(지침 §8.3)
    if r == '뇌혈관진단' and ('생명' in c or 'AIA' in c.upper()): return '뇌출혈진단비'
    # ★★★★★v657 (지점장 지시 2026.09.03 「<b>방법을 찾아내라</b>」 · 박범수·박성희 검산 불일치).
    #   [사실] 삼성 <b>「종합병원 암 전액본인부담 통합치료비」는 <b>회사담보명이 완전히 같은 줄이 3개</b>다.
    #     신정원명만 다르다 — `암수술` 1,000 / `기타` 500 / `암주요치료비` 1억.
    #   [구 동작] 세 줄이 전부 <b>하이클래스(암)로 뭉개져 11,500</b>이 됐다
    #     → 한장표는 `암수술비 1,000` · `암주요치료비 1억`로 <b>따로</b> 쓴다 → 검산 불일치.
    #   [방법] 회사담보명이 같을 땐 <b>신정원명이 유일한 구분자</b>다 — F8과 같은 근거.
    #   ★`기타`는 건드리지 않는다(종전대로 하이클래스) — 한장표에 대응 항목이 없다.
    if ('종합병원' in r) and ('통합치료비' in r) and ('암' in r):
        if s2 == '암수술':      return '암수술비'
        if s2 == '암주요치료비': return '암주요치료비'
    ...
    # ★★★★★v653 (지점장 지적 2026.09.03 「메리츠 = 가입담보 상세 List 이거 보자」 · 박성희).
    #   [실측] 메리츠 담보명은 <b>`상해입통원의료비`</b>다 — <b>「입통원」에는 「입원」이라는
    #   연속 문자가 없다</b>. 그래서 F7이 안 걸리고 뒤의 「통원」만 보여 <b>통원 행</b>으로 갔고,
    #   통원에 5,000만이 들어가 <b>「일당 행에 100만원 초과」 게이트</b>에 걸려 [확인]큐로 빠졌다.
    #   ⇒ 메리츠 계약이 <b>담보 0건</b>이 됐다(실측 C8 0건). 제9조 5항과 같은 통합형이다.
    if '의료비' in r and (('입원' in r and '통원' in r) or '입통원' in r):
        return '실손입원의료비'
...
def _sj_rows(block):
    ...
    while i < len(block):
        ...
        rest = h.group(3); amt_s = None; _sjw = ''       # ★v657
        m = _SJ_AMT.search(rest)
        if m:
            amt_s = m.group(1); rest = rest[:m.start()]
        else:                                            # 신정원담보명 wrap → 다음 1~2줄에서 금액 회수
            # ★★★★★v657 (지점장 지시 2026.09.03 「방법을 찾아내라」).
            #   [사실] 담보명이 길면 메리츠 별첨은 <b>두 줄로 쪼개진다</b>.
            #     `14 정액 [건강]…통합치료비(표준형,연간1억원`
            #     `                    암수술              1,000만`  ← <b>신정원명이 다음 줄</b>
            #   [구 동작] 다음 줄에서 <b>금액만</b> 가져오고 신정원명은 버렸다 → `s2`가 빈 문자열.
            #     회사담보명이 <b>완전히 같은 3줄</b>(암수술·기타·암주요치료비)을 가를 수가 없어
            #     <b>전부 하이클래스(암) 11,500</b>으로 뭉개졌다(검산 불일치).
            #   ⇒ <b>금액과 함께 신정원명도 가져온다</b>.
            for k in (1, 2):
                if i + k < len(block):
                    c = _SJ_CONT.match(block[i + k])
                    if c:
                        amt_s = c.group(1)
                        _cp = [p for p in re.split(r'\s{2,}', block[i + k].strip()) if p]
                        if _cp and not _SJ_AMT.search(_cp[0]):
                            _sjw = re.sub(r'\s+', ' ', _cp[0].strip())
                        break
        ...
        if not sj and _sjw: sj = _sjw                    # ★v657 다음 줄에서 회수한 신정원명
    ...
    fixed = []
    # ★★★★★v665 제156조 (장유진 DB 실측 2026.09.04) — <b>회사담보명·신정원명·금액이 전부 같은 줄</b>(`중대상해수술비(뇌,내장손상)` 1,000 ×2)은
    #   위 v44 규칙이 둘 다 신정원명 '특정상해수술'로 갈아타 → resolve 불가 → <b>두 줄 다 조용히 사라졌다</b>(엑셀·확인큐 어디에도 없음).
    #   ⇒ 첫 줄은 회사담보명 그대로 살리고, 두 번째부터는 `[확인] 중복행` 접두로 확인큐에 올린다. 합산(2,000)인지 표 중복인지는 지점장이 본다.
    _seen665 = {}
    for nm, sj, v in out:
        _k665 = (nm, sj, v)
        if cnt.get(nm, 0) > 1 and sj and sj != nm:
            _dupfull = sum(1 for _o in out if (_o[0], _o[1], _o[2]) == _k665) > 1
            if _dupfull:
                _seen665[_k665] = _seen665.get(_k665, 0) + 1
                if _seen665[_k665] == 1: fixed.append((nm, v))
                else: fixed.append(('[확인] 중복행%d %s' % (_seen665[_k665], nm), v))
                continue
...
_JN_GUBUN = re.compile(r'^(?:기본|선택)\s+(?:기본|선택|납입면제|진단|치료|암|수술|입원|통원|일당|사망|후유장해|배상|비용|간병|실손|재물|운전자|기타)?\s*')   # ★v744 흥국 구분2열 '선택 암'(조헌일 실측) — 구 목록에 '암'이 없어 담보명이 `암암진단비`가 됐다
...
_JN_CO2 = (('KB','KB'),('한화','한화'),('농협','NH농협'),('NH','NH농협'),('DB','DB'),
           ('현대해상','현대해상'),('흥국','흥국'),('흥Good','흥국'),('Heungkuk','흥국'),  # ★v744 조헌일 실측: 표지 상품명이 `무배당 흥Good…`이라 '흥국'이 없다
           ('롯데','롯데'),('let:','롯데'),
           ('삼성','삼성화재'),('메리츠','메리츠'))
...
# ★★★★★v745 제170조 (지점장 확정 2026.09.18 「주요치료비·통합치료비·10억통장은 다 <b>연간 1회 각각</b> 지급이고
#   <b>수술은 매회</b>다 — 제안서를 다시 봐라」). 제안서 4건 전문을 다시 읽어 확인:
#     KB 673 3대질환 주요치료비 「지급방식 : 수술은 수술1회당 보장, 나머지 치료는 각각 연간1회에 한하여 보장」
#     KB 530 암 통합치료비Plus 「보장항목별로 … 연간 1회에 한하여 지급 (단, 암수술·다빈치로봇 암수술은 <b>수술 1회당</b> 지급)」
#     흥국 순환계 주요치료비Ⅴ 「_관혈수술비…(<b>수술당</b>)」 / 나머지 「(연간1회한)」 · 현대 심뇌혈관 주요치료비Ⅱ 전 항목 「(연간1회한)」
#   ⇒ 「하나만 준다」 판정은 폐기한다. 실제 제안서 4건 어디에도 그런 문구가 없었고(「중 하나」는 전부 <b>보험료 납입면제</b> 문구),
#     내가 만든 가상 문구로만 검증했던 규칙이었다. 이제 지급 주기만 가린다 — <b>매회</b>(수술 1회당) / <b>연1회</b> / 확인.
_PAY_EVERY = re.compile(r'수술\s*1?\s*회\s*당|수술당|매\s*회|매회지급|1사고당|사고당')
_PAY_YEAR  = re.compile(r'연간\s*\d*\s*회\s*한?|연\s*1\s*회|각각\s*연간|치료별\s*연간')
...
def jean_split_tag(name):
    ...
    if '암주요치료' in t and '_' in t:           # ★v744 암 치료별 담보를 먼저 가른다(아래 순환계 규칙이 '중환자실'을 먼저 집지 않게)
        ...
        if _ca:
            return {'organ': '암', 'treat': {'암수술': '암수술', '항암방사선': '방사선', '항암약물': '약물',
                                             '항암호르몬약물': '호르몬', '암중환자실': '암중환자실'}[_ca.group(1)]}
    m = re.search(r'\((심장질환|뇌혈관질환|심뇌혈관질환)?(비관혈수술|관혈수술|중환자실입원|중환자실|혈전용해치료|혈전용해|혈전제거)[^()]*\)\s*$', t)
    if not m:
        # ★v744 흥국(조헌일 실측): `주요치료비Ⅴ_관혈수술비(혈전제거제외)(수술당)` 처럼 <b>밑줄 뒤</b>에 치료가 오고 괄호가 더 붙는다
        _u = t.split('_', 1)
        if len(_u) == 2:
            m = re.search(r'(심장질환|뇌혈관질환|심혈관및기타질환|심뇌혈관질환)?\s*(비관혈수술|관혈수술|중환자실|혈전용해|혈전제거)', _u[1])
            if (not m) and ('암주요치료' in t):
                # ★v744 흥국 `상급종합병원Ⅱ 암주요치료비Ⅱ(유사암제외)_암수술비(연간1회한)` — 암도 치료별로 담보가 쪼개진다
                _m2 = re.search(r'(암중환자실|암수술|항암방사선|항암호르몬약물|항암약물)', _u[1])
                if not _m2 and '중환자실' in _u[1] and '암' in t: _m2 = re.match(r'(암중환자실)', '암중환자실')
                if _m2:
                    return {'organ': '암', 'treat': {'암수술': '암수술', '항암방사선': '방사선', '항암약물': '약물',
                                                     '항암호르몬약물': '호르몬', '암중환자실': '암중환자실'}[_m2.group(1)]}
    if not m: return None
    organ = {'심장질환': '심', '뇌혈관질환': '뇌', '심혈관및기타질환': '심'}.get(m.group(1) or '', '공통')
    tr = m.group(2)
    ...
    try:
        ...
        for x in (rows or []):
            ...
            for i in _hits[1:]:
                ...
                _e = bool(_PAY_EVERY.search(_w)); _o = bool(_PAY_YEAR.search(_w))
                # ★v742 KB 특별약관 세부보장항목 실물(인포메이션 35~37쪽): 보장항목마다 지급방식 칸에
                #   「연간1회한 · 수술1회당」이 따로 적힌 표다 → 항목별로 각각 지급(가입금액은 연간 총한도).
                # ★v745 제170조 — 통짜 주요치료비·통합치료비는 <b>항목마다 연간 1회</b>가 원칙이고,
                #   그 안의 <b>수술 항목만 수술 1회당(매회)</b>이다(KB 673·530 원문). 그래서 둘을 따로 담는다.
                if _o or _e: mode = '연1회'
                else: continue
                _surg = '매회' if _e else ''
                src = _w[:220]; break
            ...
            if _sp:
                _it.update(_sp); _it['src'] = '담보명: ' + nm[-34:]
                # ★v745 담보명 자체에 주기가 적혀 있으면 그것이 답이다(흥국 `(수술당)` vs 현대 `(연간1회한)`)
                if _PAY_EVERY.search(key): _it['mode'] = '매회'
                elif _PAY_YEAR.search(key): _it['mode'] = '연1회'
...
def parse_txt(txt, filename='', extra=None):
    ...
    # ★★★★★v656 (지점장 지시 2026.09.03 「메리츠 인식하자」) — KB와 동일한 자리에 붙인다.
    #   메리츠는 「보장별 상세 내역」이 한장표 대용이다. 없으면 {} — 기존 동작 불변.
    if not _hjkb:
        try: _hjkb = parse_mz_summary(lines)
        except Exception as _e656:
            _hjkb = {}; print('[v656 메리츠요약표] 실패:', _e656)
...
def resolve_kw(raw):
    # ■■■★★★★★[제154조 — 2026.09.05 지점장 실측 지적 7건. 영구]
    #   조영선 건에서 한꺼번에 터졌다. 전부 <b>「마스터에 행이 없으면 [확인]큐」</b>를 어긴 결과다.
    _v154 = re.sub(r'\s', '', str(raw or ''))

    # ① (폐기 2026.09.06) 교통○○ → [확인] 규칙은 지점장 지시가 아니었다. 종전 매핑으로 복귀.

    # ② <b>「6주이상」은 6주미만 행이 아니다</b> — 실사고: 음주·무면허·뺑소니 / 중과실 (6주이상) 3,000이
    #    6주미만 행에 들어가 3,000이 됐다. 진짜 6주미만은 500이었다.
    if '6주이상' in _v154 or '6주 이상' in str(raw or ''):
        return (None, 0)

    # ③ <b>다빈치로봇수술비는 암수술이 아니다</b> — 실사고: 암수술 200 + 다빈치 1,000·500 = 1,700.
    if '다빈치' in _v154 or '로봇수술' in _v154:
        return ('다빈치로봇수술비', 0)
    ...
    #     → '고액'+'암'+'진단'이면 전부 고액암 행.
    # ★★★★★v696 (지점장 실측 2026.09.15 김O민 「고액암이 없는데 1,000이 나온다」):
    #   KB `통합전이암진단비(3대특정고액전이암진단비)`가 '고액'+'암'+'진단'에 걸려 <b>고액암</b>으로 갔다.
    #   전이암 담보는 형제 6개와 같이 <b>통합전이암</b>이다 — 고액암 규칙보다 앞선다.
    if '전이암' in _v154 and '고액' in _v154:
        return ('통합전이암', 0)
    if ('고액암' in _v154) or ('고액' in _v154 and '암' in _v154 and '진단' in _v154):
        return ('고액암', 0)

    # ⑤ <b>질병특정고도장해재활치료비 = 질병후유80%</b>(지점장 확정 2026.09.05).
    if '고도장해' in _v154 and ('재활' in _v154 or '치료비' in _v154):
        return ('질병후유80%', 0)

    # ⑥ <b>상해의료비는 상해의료비 칸에</b>. 입원실손·통원에 넣지 않는다. 이 담보만 100% 비갱신.
    if _v154 in ('상해의료비','상해의료비담보','상해의료비특약') or _v154.startswith('상해의료비'):
        return ('상해의료비', 0)
    ...
    # ★★★★★v655 (지점장 지시 2026.09.03 「<b>전이암 표적치료비를 표적치료비에 잡지 마라</b> · 오늘 그러더라」).
    #   [원인] `has('표적')` 하나만 보아 <b>전이암이 붙어도 그대로 표적항암치료비 행</b>으로 갔다.
    #   전이암은 <b>별도 담보</b>다(마스터 `통합전이암` 행은 <b>진단비</b> 전용) — 치료비 변형은
    #   마스터 무행이므로 <b>[확인]큐</b>로 보낸다(제0조 — 없는 행에 임의 삽입 금지).
    #   ★세기조절·양성자를 표적 앞으로 올린 v215와 <b>같은 방식</b>.
    if has('표적') and has('전이암'): return None,0
    ...
    if has('후유') or has('장해') or has('장애'):
        ...
        # ★★★v665 (지점장 확정 2026.09.04): <b>「(20~100%)」도 20% 후유장해다</b> — v222 정규식이 `20%`만 보고
        #   `20~100%`(20 뒤에 ~)를 놓쳐 <b>일반상해후유장해(20~100%) 100 + (3~100%) 100 = 200</b>으로 합산됐다.
        #   마스터 행은 3%·80%뿐 → 20~100%·50~100%도 [확인]큐. (3~100%)는 종전대로 3% 행.
        if re.search(r'(?<![\d.])(20|50)\s*(?:%|[~\-–]\s*100\s*%)', n) and not re.search(r'(?<![\d.])80\s*%', n):
            return None,0
...
# ★★★★★v527 제110조 <b>폐기</b> (지점장 지시 2026.08.21 「요양병원은 삭제다」)
    ...
    #   (척추·중대·골반·대퇴·상지·하지·손목·발목·부위·특정)은 넣을 행이 없다 → [확인]큐. 「엑셀에 없는 건 하지 마라」(제0조).
    #   실측: `척추골절진단비`가 아래 `has('골절') and has('진단')`에 걸려 골절(치아파절포함)에 200이 더해졌다.
    if has('골절') and no('5대골절') and any(has(_k) for _k in ('척추','중대','골반','대퇴','상지','하지','손목','발목','부위','특정')):
        return None,0
    ...
    # ★★★★★v665 제155조 (지점장 확정 2026.09.04 장유진 DB 「깁스는 50이다」) — <b>「신깁스치료비」는 깁스진단비가 아니다</b>.
    #   실측: 같은 계약에 `신깁스치료비(상해및질병)` 200 + `깁스치료비` 50이 둘 다 100행으로 가서 <b>250</b>이 됐다.
    #   마스터 깁스 행은 반깁스(99)·깁스진단비(100) 둘뿐 — 신깁스 행은 없다 → [확인]큐(제0조).
    if has('신깁스'): return None,0
...
def resolve2(raw):
    ...
    if ('산정특례' in _n300) and ('뇌' in _n300) and ('수술' not in _n300):
        return ('산정특례뇌혈관', 0)
    # ★★★★★v665 제155조 2항 (장유진 실측 2026.09.04) — DMAP 부분일치 선점 차단 3건. 정답지(KB 한장표) 기준.
    #   ㉠ `신깁스치료비(상해및질병)` 200 — DMAP 키 '깁스치료'가 substring으로 잡아 깁스진단비 250(정답 50).
    #   ㉡ `신특정순환계질환진단비` 10 — 순환계 묶음 규칙에 걸려 뇌혈관·협심증·급성심근 3행에 10씩 얹혔다(정답지 3,000/3,000/1,000).
    #   ㉢ `자궁적출수술비(여성생식기의암ㆍ제자리암)` 100 · `유방암으로인한유방수술비` 50 — 암수술 행에 산입(정답지 암수술비 1,300에 없음).
    #   셋 다 마스터에 행이 없다 → [확인]큐(제0조). v350·v560과 같은 뿌리(부분일치 선점).
    _r665 = re.sub(r'\s', '', str(raw))
    if ('신깁스' in _r665) or ('신특정순환계질환진단' in _r665) or ('자궁적출' in _r665) or ('유방수술' in _r665):
        return (None, 0)
    if raw in DMAP:
        v = DMAP[raw]
    ...
    if ('가족동승' in raw) or ('동승가족' in raw): return (None, 0)   # ★v665 DB 표기 '동승가족' 추가(자부상 45→40, 정답지 40)
...
def _dedup_std(raw):
    ...
    if '진단' in _rn and '수술' not in _rn and '주요치료' not in _rn:
        if ('심장질환진단' in _rn) and ('허혈' not in _rn) and ('급성심근' not in _rn):
            # ★★★★★v702 (지점장 확정 2026.09.15 「메리츠는 심장질환진단비 자체가 없다」):
            #   `심장질환진단비Ⅰ/Ⅱ`는 <b>정본표에 없는 담보</b>다. 구 v384는 이것을 협심증·급성심근으로
            #   임의 배정했다 — 표에 없는 묶음을 유추해 채운 것(심장 유동성 조문 ② 위반).
            #   ⇒ 채우지 않는다 → resolve가 처리(마스터 무행이면 [확인]큐).
            pass
        if '뇌질환진단' in _rn:
            _mn=_rmn(_rn)
...
def build_excel(data, out):
        ...
        for raw, amt in dambo.items():
            ...
            if ('진단' in _rn and '수술' not in _rn and '주요치료' not in _rn
                ...
                if _is_solo5:
                    ...
                    #   ★★★★★v702 (지점장 확정 2026.09.15 「<b>메리츠는 심장질환진단비 자체가 없다</b>」):
                    #     `심장질환진단비Ⅰ/Ⅱ`는 <b>정본표에 없는 담보</b>다. 위 v384 배정(Ⅰ→협심증 / Ⅱ→급성심근)은
                    #     표에 없는 묶음을 유추해 채운 것 — 「표에 없는 회사·없는 묶음은 유추해서 채우지 않는다 → [확인]」
                    #     (심장 유동성 조문 ②·꼭대기 지침) 정면 위반. ⇒ 채우지 않는다.
                    pass
                ...
                # ★★★★★v665 (장유진 실측 2026.09.04) — KB `신특정순환계질환진단비` 10만은 순환계3대 묶음이 아니다(정답지: 뇌혈관 3,000·급성심근 1,000에 10 없음).
                #   아래 `'순환계' in _rn` 인라인 묶음에 걸려 협심증·급성심근·뇌혈관 3행에 10씩 얹혔다 → 묶음 진입 차단, [확인]큐(제0조).
                elif '신특정순환계' in _rn:
                    _heart_bundle = []
                elif '순환계' in _rn:
                ...
                # ===== BARUM 10사 질병코드 분류표 정본(2026.07.05 지점장 확정): 특정Ⅰ/Ⅱ 라벨=회사마다 다름 → 회사별 표대로 =====
                    ...
                    _t=_rmn(_rn)
                    # ★★★★★v696 (김O민 KB 실측 「협심증·심부전·빈맥이 1,500」): `부정맥질환(Ⅰ49)진단비` 500이
                    #   로마자 Ⅰ을 등급 1로 읽혀 <b>특정Ⅰ 묶음</b>(협심·빈맥·심부전)에 얹혔다. 부정맥 행은 공란이었다.
                    #   ⇒ 담보명이 <b>부정맥</b>이면 등급 묶음을 타지 않는다(제154조 ②·꼭대기 지침).
                    if ('부정맥' in _rn) and ('제외' not in _rn):
                        _t = 0
                    _i49excl=('제외' in _rn) and (('I49' in _rn) or ('부정맥' in _rn))   # ★(기타심장부정맥제외)=Ⅰ에서 I49 뺀 묶음(부정맥 담보 아님)
                    _rn49 = _rn.replace('Ⅰ','I').replace('ⅰ','I')
                    _i49=(not _i49excl) and (('I49' in _rn49) or ('기타부정맥' in _rn49) or ('기타심장부정맥' in _rn49)
                                             or ('부정맥' in _rn49 and '49' in _rn49))
            ...
            # ★★★★★v682 제154조 ⑥ (조영선 흥국 0804 44,650 실측): resolve2 v590은 `의료비(입원+통원)`을 세대 불문 <b>입원</b>으로
            #   보낸다(4세대 정본). 그런데 <b>1세대</b>(가입 2009.09 이전)의 `상해 …의료비(입원+통원)`은 v95 정본대로
            #   <b>상해의료비 칸</b>이다. 세대는 계약 단위로만 알 수 있어 <b>여기</b>서 판정한다(resolve2는 계약을 모른다).
            if std == '입원':
                _n682 = re.sub(r'\s', '', str(raw))
                if ('의료비' in _n682) and ('입원' in _n682) and ('통원' in _n682) and ('상해' in _n682):
                    try: _g682 = silson_gen(ct.get('contract_date',''), amt, ct.get('product',''), False, False)
                    except Exception: _g682 = ''
                    if str(_g682).startswith('1세대'):
                        print(f"[v682 상해의료비] {ct['company']} '{raw}' {amt} → 상해의료비 칸(1세대 통합형 · 입원 아님)")
                        std = '상해의료비'
    ...
    if _sbc:
        for _ci2, _c2 in enumerate(contracts):
            ...
            if _filled and not _partial:           # ★재구성: 별첨 잔재 제거 후 세부값으로 채운다
                # ★v682 (조영선 라이나 실측): 재구성이 별첨 `(H)고액암진단 2,000`(고액암 행)까지 지웠는데 세부표엔 고액암 칸이
                #   없어 <b>영영 사라졌다</b>. 세부표가 대신 채울 수 있는 행(화이트리스트·사망·세부 키)만 지운다.
                _rebuild_ok = set(_SEBU_WIN_682) | {'일반사망','질병사망(80세)'} | set(_vals.keys())
                for _r2 in _filled:
                    _nm682 = str(ws.cell(_r2,2).value or '').strip()
                    if _nm682 in _rebuild_ok: ws.cell(_r2,_cl2).value = None
                    else: print(f"[v682 재구성] {_cn2} 「{_nm682}」 별첨값 보존(세부표 무칸)")
            ...
            for _nm2,_v2 in _vals.items():
                ...
                if _tgt in ('입원','통원','약값','상해의료비','MRI/도수치료/비급여주사'):
                    if not _is_silson_like(_c2.get('company',''), _c2.get('product',''), _c2.get('dambo')):
                        ...
                        continue
                # ★v682 제154조 ⑥: 1세대 통합형은 상해의료비 칸 하나다. 세부가입현황은 같은 100을 입원·통원 양쪽에
                #   싣는다 → 상해의료비에 별첨값이 있으면 입원·통원·약값 세부보충은 하지 않는다(이중기재 차단).
                if _tgt in ('입원','통원','약값'):
                    _rsm682 = nm2r.get('상해의료비')
                    if _rsm682 and ws.cell(_rsm682,_cl2).value not in (None, ''):
                        print(f"[v682 세부보충] {_cn2} {_tgt} {_v2} 건너뜀 — 상해의료비 별첨값 있음")
                        continue
                if _partial:
    ...
    def _qclass(_raw, _note):
        ...
        if ('후유' in _n) and ('80' not in _n) and re.search(r'(?<![\d.])(20|50)\s*(?:%|[~\-–]\s*100\s*%)', _n):
            return ('규칙제외', '후유장해 20%·50%·20~100% = 미기재(v222·v665)')
    ...
    try:
        ...
        if _hj:
            ...
            _pairs = [
                ...
                # ★v682: 한장표(조영선)는 1세대 `의료비(입원+통원)` 100을 상해입원 3,100·상해통원 110 <b>양쪽</b>에 접어 넣는다.
                #   BARUM은 상해의료비 단독 행이 정본 → v292 선례대로 <b>검산식만</b> 합산.
                ('입원',              _g('입원','상해의료비')),
                ('통원',              _g('통원','약값','상해의료비')),
            ]
...
function reDL(k){const f=savedFiles[k];
  ...
  if(!f||!f.b64){alert("파일이 사라졌습니다. 다시 분석해 주세요.");return;}
  try{ dl(b64toBlob(f.b64,f.mime),f.name); }
  catch(e){ alert("휴대폰에서 이 방식이 막혔습니다. 다시 분석한 뒤 바로 누르세요."); }}
$("#send").onclick=async()=>{
...
def _pwa_icon(size=192):
    ...
    # ★★★★★v638 (지점장 지시 2026.09.02 「저기 아이콘 그대로 써줘 · 보장분석 글자삭제」).
    #   도시배경·글자 버전(v633·v634) <b>전부 폐기</b>. 코드로 그리는 <b>M 로고</b>로 되돌린다.
...
def _icon(sz: int):
    ...
    try:
        # ★★★★★v637 (지점장 실측 2026.09.02 「아이콘 안 바뀌」)
        #   아이콘을 <b>하루(86400초) 캐시</b>하고 있었다. 코드를 바꿔 배포해도
        #   브라우저·설치앱은 <b>어제 받은 그림을 계속 쓴다</b>.
...
def doctrine_robot(heavy=False):
    ...
    def _ck166():
        _full = ('요약\n 1 암(유사암제외) 주요치료비Plus 3,000만원\n 2 신특정순환계질환 주요치료비Plus 1,000만원\n 3 비급여 암 주요치료비 2,000만원\n'
                 ...
                 ' 신특정순환계질환 주요치료비Plus\n  수술은 수술 1회당, 나머지 치료는 각각 연간1회한\n'
                 ' 비급여 암 주요치료비\n  비급여 항암치료를 받은 경우 지급\n')
        _rows = [{'name': '암(유사암제외) 주요치료비Plus', 'amt': 3000}, {'name': '신특정순환계질환 주요치료비Plus', 'amt': 1000},
                 {'name': '비급여 암 주요치료비', 'amt': 2000}, {'name': '질병수술비', 'amt': 10}]
        _g = [(o['mode'], o.get('surg', '')) for o in jean_paymode(_full, _rows)]
        return '' if _g == [('연1회', ''), ('연1회', '매회'), ('확인', '')] else '판정 %s (정답 연1회 · 연1회+매회 · 확인)' % _g
    ...
    def _ck167():
        ...
        _g = [(o['mode'], o.get('surg', '')) for o in jean_paymode(_full, [{'name': '암 통합치료비Plus', 'amt': 5000}])]
        if _g != [('연1회', '매회')]: return '세부보장항목 표 판정 %s (정답 연1회+수술 매회)' % _g
    ...
    # ★v744 제169조 — 흥국 제안서(조헌일 실측): 구분2열 '선택 암' · 밑줄 뒤 치료 · 암 치료별 담보 · 표지 '흥Good'
    def _ck169():
        if _jn_strip_gubun('선택  암   암진단비(유사암제외)') != '암진단비(유사암제외)': return "구분2열 '선택 암' 제거 실패"
        _c = [('순환계질환(특정질병제외)주요치료비Ⅴ_관혈수술비(혈전제거제외)(수술당)', {'organ': '공통', 'treat': '관혈'}),
              ('순환계질환(특정질병제외)주요치료비Ⅴ_뇌혈관질환혈전용해치료비(연간1회한)', {'organ': '뇌', 'treat': '혈전용해'}),
              ('순환계질환(특정질병제외)주요치료비Ⅴ_심혈관및기타질환혈전용해치료비(연간1회한)', {'organ': '심', 'treat': '혈전용해'}),
              ('상급종합병원Ⅱ 암주요치료비Ⅱ(유사암제외)_암수술비(연간1회한)', {'organ': '암', 'treat': '암수술'}),
              ('상급종합병원Ⅱ 암주요치료비Ⅱ(유사암제외)_암중환자실치료비(연간1회한)', {'organ': '암', 'treat': '암중환자실'}),
              ('심뇌혈관질환주요치료비Ⅱ(관혈/비관혈)(연간1회한)(간편건강고지)(심장질환관혈수술)', {'organ': '심', 'treat': '관혈'})]
        for _n, _w in _c:
            if jean_split_tag(_n) != _w: return '%s → %s (정답 %s)' % (_n[-18:], jean_split_tag(_n), _w)
        return ''
    _ck('제169조 흥국제안서', '제169조', _ck169)

    # ★v745 제170조 — 지급 주기: 연간 1회 각각이 원칙, 수술만 매회. 「하나만」 판정은 폐기했다.
    def _ck170():
        if '_PAY_ONE' in globals(): return "'하나만' 판정이 아직 살아 있다"
        _t = ('요약\n 1 3대(간,폐,신장)질환 주요치료비 500만원\n 2 순환계 주요치료비Ⅴ_관혈수술비(수술당) 1,000만원\n'
              '보장내용\n 3대(간,폐,신장)질환 주요치료비\n  ※ 지급방식 : 수술은 수술1회당 보장, 나머지 치료는 각각 연간1회에 한하여 보장\n'
              ' 순환계 주요치료비Ⅴ_관혈수술비(수술당)\n  관혈수술을 받은 경우 수술 1회당 지급\n')
        _r = [{'name': '3대(간,폐,신장)질환 주요치료비', 'amt': 500}, {'name': '순환계 주요치료비Ⅴ_관혈수술비(수술당)', 'amt': 1000}]
...
async def analyze(file:UploadFile=File(None), file2:List[UploadFile]=File(None), pw:str=Form('')):
    ...
    try:
        ...
        try:
            ...
            # ★★★★★v636 (지점장 실측 2026.09.02 「모바일에서 엑셀만 다운된다」)
            #   <b>원인</b>: 5개를 <b>하나의 try</b>로 묶어 돌려서, 첫 번째(엑셀) 다음에서
            #   한 번 예외가 나면 <b>나머지 4개는 URL이 아예 안 만들어졌다</b>.
            #   URL이 없으면 프론트가 blob 폴백으로 가는데, <b>모바일은 blob을 막아</b>
            #   카드를 눌러도 <b>아무 반응이 없다</b>.
            #   <b>해법</b>: 파일별로 따로 감싸 <b>하나가 죽어도 나머지는 산다</b> + 실패 사유를 남긴다.
            _dlerr = []
            for _bk, _nk, _uk in (('xlsx_b64','xlsx_name','xlsx_url'),
                ...
                try:
                    ...
                    # ★파일명에 경로 구분자·제어문자가 섞이면 open 자체가 터진다.
                    _fn = os.path.basename(str(_fn)).replace('\\', '_')
                    _fn = ''.join(c for c in _fn if ord(c) >= 32) or (_bk + '.bin')

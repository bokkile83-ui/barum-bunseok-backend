...
# ★★★★★v365 (지점장 지시 2026.08.06, 영구): 1-3종 종수술비 <b>강제 슬래시 표기</b>.
#   지점장 원문 = "1-3종 수술비 100 = 20/50/100 ← 질병·상해에 각각 강제값을 넣어라".
#   ★[확인] 금액 무관 고정값이다. 다른 금액일 때의 처리는 지시에 없어 정하지 않는다(제0조).
_SURG13_FORCE = '20/50/100'
    ...
    try:
        ...
        for l in lines:
            for m in re.finditer(r'(?<!\S)[\d][\d,]{0,11}(?!\S)', l):
                # ★★★★★v365 (지점장 지시 2026.08.06 "이영태 정답은 심혈관Ⅰ=협심증·심부전·빈맥", 영구):
                #   <b>가입금액 앞에는 반드시 담보명(한글)이 온다</b>. 앞에 한글이 하나도 없는 숫자는
                #   <b>금액이 아니라 담보명 자체의 일부</b>다 → 열 경계 후보에서 제외한다.
                #   [실측 이영태 현대 Hi2007] 별첨에 `5 대혈관질환진단（4대혈관질환）…` 같은 줄이 <b>2개</b> 있어
                #     줄머리 `5`(end=2)가 <b>2회 등장 = 열 경계</b>로 인정됐다 → 경계 [2, 58].
                #     그 결과 <b>모든 줄의 앞 2칸이 첫 열</b>로 떨어져 나가 담보명 첫 글자가 통째로 잘렸다:
                #     `심혈관질환（특정 I ）…` → <b>`혈관질환（특정 I ）…`</b> · `뇌혈관질환…` → `혈관질환…`
                #     · `기본계약…` → `본계약…`.
                #   → `_HB` 현대 규칙 `'특정1' in t and <b>'심' in t</b>`가 <b>'심'이 없어</b> 매칭 실패,
                #     특정Ⅰ 500이 협심증·심부전·빈맥 어디에도 안 들어가고 <b>조용히 소실</b>됐다.
                #   ★고정 좌표·최소폭 상수를 쓰지 않는다 — <b>표의 구조(담보명→금액)</b>로만 판정한다.
                if not re.search(r'[가-힣]', l[:m.start()]): continue
...
_SEBU_LAB = [
    ...
    # ★★★★★v359 (지점장 지적 2026.08.06 "허혈성 가입을 안했는데 허혈성이 표기된다", 영구):
    #   <b>'허혈성 진단비' 행 = 회사담보명 '허혈성심장질환진단비' 단독 담보 전용</b>(정본 최상위).
    #   <b>판정 근거는 회사담보명뿐이다 — 신정원담보명은 무시</b>한다.
    #   세부가입현황·한장보장표는 <b>신정원 분류표</b>이고, 신정원은 <b>KB 특정Ⅰ 같은 묶음도
    #   '허혈성심장질환진단비'로 뭉뚱그려</b> 찍는다. 그걸 그대로 허혈성 행에 보충하면
    #   <b>가입하지도 않은 허혈성 단독 담보가 생겨난다</b>(실측 김민수 KB: 별첨엔 특정Ⅰ·Ⅱ·부정맥뿐인데 허혈성 1,000).
    #   → 이 라벨로는 <b>보충하지 않는다</b>(None). 허혈성은 별첨 회사담보명 경로로만 들어온다.
    ('허혈성심장질환진단비',None,'L'), ('심장질환수술비','심장수술비','R'),
    ('급성심근경색진단비','급성심근경색','L'),
...
_SEBU_PAIRS = [
    ...
    (None,'심장수술비'), ('급성심근경색',None),   # ★v359 좌측=신정원 '허혈성심장질환진단비' → 허혈성 행 보충 금지(위 _SEBU_LAB 주석)
...
def parse_txt(txt, filename=''):
    ...
    _HB = {   # ★★★심장 묶음담보 회사별 정본표(지점장 최종본 2026.07.18 · 이전 표 폐기)
      ...
      'DB': [
        ...
        (lambda t: '특정3' in t and '심장' in t,   ['심장판막','심부전','빈맥']),   # ★v368 지점장 확정 2026.08.06: 빈맥 복원(v264 '빈맥 삭제'는 오류)
        (lambda t: '순환계3대' in t,               ['빈맥','부정맥','심부전']),
      ],
      '현대': [
        (lambda t: '특정허혈' in t,                 ['급성심근경색']),
        # ★★★★★v366 신설지침 (지점장 확정 2026.08.06, 영구):
        #   지점장 원문 = "<b>이영태의 현대해상 5대혈관질환(4대혈관질환) = 4대 혈관질환
        #   뇌혈관질환, 허혈성 심장질환, 급성심근경색증, 심혈관질환 → 새로운 지침이다.
        #   그럼 이영태는 뇌혈관에 500 / 허혈성 500 / 급성심근경색 500 각각 기재되어야한다</b>".
        #   → <b>뇌혈관진단비 · 허혈성 진단비 · 급성심근경색 3행에 각 100% 동일 금액</b>.
        #   ★<b>「심혈관질환」은 마스터에 행이 없다 → 기재하지 않는다</b>(꼭대기 지침 "엑셀에 없는 건 하지 마라").
        #     지점장이 기재 대상으로 <b>3개만 명시</b>했다. 내가 협심증 등으로 유추해 넣지 않는다.
        #   ★★<b>이 담보는 '허혈성 진단비 = 단독 전용' 원칙의 명시 예외다</b>(지점장 직접 확정).
        #     다른 묶음(특정Ⅰ·Ⅱ 등)은 종전대로 허혈성 행 금지 — 이 조문을 근거로 확장하지 말 것.
        #   ★<b>`5대혈관질환진단(죽상경화증)` 100은 지시에 없다</b> → 조건을 '4대혈관질환'으로 좁혀
        #     죽상경화증은 [확인]큐에 그대로 남긴다.
        (lambda t: '4대혈관질환' in t,             ['뇌혈관진단비','허혈성 진단비','급성심근경색']),
    ...
    for _c in deduped:
        ...
        for _k in list(_c['dambo'].keys()):
            ...
            # ★★★★★v359 (지점장 지적 2026.08.06 "부정맥은 부정맥에 가야한다", 영구):
            #   구 코드는 <b>로마자 Ⅰ→1 변환을 먼저</b> 하고 그 다음 I49를 복원하려 했다.
            #   그래서 `부정맥질환(Ⅰ49)진단비` → `부정맥질환(149)진단비` 가 되어버려
            #   뒤의 `re.sub(r'[IiⅠ]49','I49')`가 <b>이미 사라진 Ⅰ를 찾다가 실패</b>했다.
            #   결과: KB 부정맥 규칙(`'부정맥' and 'I49'`)이 <b>한 번도 매칭되지 않았다</b>.
            #   → <b>I49 고정을 로마자 변환보다 먼저</b> 한다. (ASCII 'I'는 replace 대상이 아니라 안전)
            _t2 = re.sub(r'[IiⅠl|]\s*49', 'I49', _t)
            # ★★★★★v364 (지점장 지시 2026.08.06 원인규명, 영구): <b>로마숫자 정규화가 전각만 했다</b>.
            #   구 코드는 `Ⅰ`(U+2160)만 '1'로 바꾸고 <b>라틴 대문자 I(U+0049)는 그대로 뒀다</b>.
            #   → `_HB` 조건은 `'특정1' in t`인데 원문이 `（특정 I ）`면 <b>영원히 매칭되지 않는다</b>.
            #   [실측 이영태 현대 Hi2007] 별첨 원문 = `심혈관질환（특정 I ）진단（갱신형）담보 500`
            #     · `심혈관질환（특정 II）진단（갱신형）담보 500` — <b>둘 다 라틴 I</b>.
            #     결과: 현대 특정Ⅰ 500이 <b>협심증·빈맥·심부전 어디에도 안 들어가고 통째로 소실</b>됐고
            #     [확인]큐에도 안 떴다(조용한 누락).
            #   [실측 이성준] 같은 원인으로 KB `심장질환（특정 I）`이 `_HB`를 못 타고
            #     resolve_kw v40b 구 규칙으로 흘러 <b>허혈성 행</b>에 앉았다(정본 위반).
            #   ★<b>같은 함정 3번째다</b> — v359 부정맥(Ⅰ49)도 뿌리가 같았다.
            #   ★새 매핑 규칙을 만들지 않는다. <b>기존 조문에 도달만 시킨다</b>(제0조).
            #   ★긴 것부터 치환해야 한다(III→3을 먼저 하지 않으면 I·I·I로 쪼개진다).
            #   ★I49는 위에서 이미 고정했으므로 안전하다.
            for _rm_src, _rm_dst in (('Ⅲ','3'),('Ⅱ','2'),('Ⅰ','1'),('III','3'),('II','2')):
                _t2 = _t2.replace(_rm_src, _rm_dst)
            # 라틴 단독 I → 1 (앞뒤에 영문자가 없을 때만. 'I49'·'AI' 등 보호)
            _t2 = re.sub(r'(?<![A-Za-z0-9])I(?![A-Za-z0-9])', '1', _t2)
...
def resolve_kw(raw):
    ...
    # ★★★★★v365 신설지침 (지점장 지시 2026.08.06, 영구): 아래 2건은 <b>담보명 그대로 지정된 행</b>에 넣는다.
    #   지점장 원문 ①"<b>일반상해의료비담보 = 상해의료비 에 넣어야 한다</b>"
    #             ②"<b>암진단치료비담보 = 엑셀 '일반암' = ppt 2건도 일반암진단비에 넣어야함 신설지침</b>"
    #   ★<b>최상단에 둔다</b> — 아래 실손 규칙(`상해입원의료비`→입원)·암 규칙이 먼저 먹으면 도달하지 못한다
    #     (DMAP 부분일치 선점 = 구조적 결함, 실측 2건).
    #   ★엑셀 원천 1곳만 고치면 보장분석지 PPT·보장진단서·설명서에 자동 반영된다(§1 등식).
    try:
        _r365 = re.sub(r'\s', '', str(raw or ''))
        if '일반상해의료비' in _r365: return '상해의료비', 0
        if '암진단치료비' in _r365:   return '일반암', 0
    except Exception:
        pass
    ...
    if has('심장질환') and has('특정') and has('진단') and no('수술','주요치료'):
        ...
        # ★★★★★v364 (지점장 확정 2026.08.06, 영구): "<b>묶음은 허혈성 행에 절대 넣지 않는다 — 맞다.
        #   허혈성은 단독이다</b>." → 구 v40b `if _is1 and not _is2: return '허혈성 진단비',0`을 <b>폐기</b>한다.
        #   [정본 §8.3.1] '허혈성 진단비' 행 = 회사담보명 <b>'허혈성심장질환진단비' 단독 담보 전용</b>.
        #     특정Ⅰ·Ⅱ 같은 <b>묶음</b>은 구성질환 행으로 분해하고 허혈성 행엔 넣지 않는다
        #     (I20·I24·I25는 '협심증' 행으로 표현).
        #   [실측 이성준] KB `심장질환（특정 I）진단비 III` 1,000이 이 줄 때문에 <b>허혈성 행</b>에 앉았다.
        #     같은 담보가 김수영(전각 Ⅰ)에서는 `_HB`로 정상 분해됐다 — <b>표기 하나로 결과가 갈렸다</b>.
        #   ★묶음 분해는 `_HB` 한 곳에서만 한다(v266 영구원칙). 여기서 하지 않는다.
        #   ★`_HB`가 못 잡으면 <b>[확인]큐</b>로 보낸다 — 엉뚱한 행에 넣는 것보다 안전하다(꼭대기 지침).
        if _is1 and not _is2: return None,0
        return None,0   # ★v364 구분 불가도 [확인]큐(구 '보수적으로 급성심근경색' 폐기 — 추측 금지)
...
def build_excel(data, out):
    ...
    for i, ct in enumerate(contracts):
        ...
        for raw, amt in dambo.items():
            ...
            if ('진단' in _rn and '수술' not in _rn and '주요치료' not in _rn
                ...
                # ★★★★★v368 (지점장 확정 2026.08.06): <b>「순환계 5종·4종·3종」 같은 담보는 없다</b>.
                #   지점장 원문 = "`순환계`+`5종` / `4종` / `3종` … <b>이런건없다</b>".
                #   → 세 분기를 <b>삭제</b>한다. 근거 없는 분해가 조용히 값을 만들어내던 자리다.
                #   ★`4대순환계` 차단(위)은 유지한다 — DB에 실재하는 담보이고(v263 지점장 확정),
                #     차단을 지우면 아래 `elif '순환계'`가 다시 잡아 v266 중복 사고가 재발한다.
                # ★순환계3대(허혈성심장 I20~25 + 뇌혈관 I60~69 + 말초 I70~79) — 삼성·DB 등. 말초=전용행無(누락 감수)
                    ...
                    if ('DB' in _co) or ('디비' in _co):
                        ...
                        _heart_bundle = ['협심증','급성심근경색','뇌혈관진단비']   # ★v367 지점장 확정 2026.08.06: 허혈성 제거(묶음은 허혈성 행 금지)
                # ===== BARUM 10사 질병코드 분류표 정본(2026.07.05 지점장 확정): 특정Ⅰ/Ⅱ 라벨=회사마다 다름 → 회사별 표대로 =====
                    ...
                    _t=_rmn(_rn)
                    # ★★★★★v359 (지점장 지적 2026.08.06, 영구): <b>로마자 Ⅰ49를 I49로 먼저 정규화</b>한다.
                    #   구 코드는 원문 `_rn`에서 <b>ASCII 'I49'만</b> 찾았다. 그런데 KB 별첨 원문은
                    #   `부정맥질환(<b>Ⅰ</b>49)진단비(맞춤고지)`처럼 <b>로마숫자 Ⅰ(U+2160)</b>을 쓴다.
                    #   → `_i49=False`가 되고, 동시에 `_rmn()`이 그 Ⅰ를 <b>등급 1</b>로 읽어
                    #     <b>KB 특정Ⅰ 묶음으로 오분해</b>됐다(실측 김민수: 부정맥 500이 협심증·허혈성·빈맥·심부전으로).
                    _rn49 = re.sub(r'[IiⅠl|]\s*49', 'I49', _rn)
                    _i49excl=('제외' in _rn49) and (('I49' in _rn49) or ('부정맥' in _rn49))   # ★(기타심장부정맥제외)=Ⅰ에서 I49 뺀 묶음(부정맥 담보 아님)
                    _i49=(not _i49excl) and (('I49' in _rn49) or ('기타부정맥' in _rn49) or ('기타심장부정맥' in _rn49))
                    if _i49 or _i49excl: _t = 0   # ★Ⅰ49의 'Ⅰ'는 등급 표기가 아니다 — _rmn 오독 차단
                    # 흥국·롯데: 특정Ⅰ=급성심근 / 특정Ⅱ=협심증+허혈+염증 / 롯데 15대=판막·심근병·빈맥·심부전
                    if ('흥국' in _co) or ('롯데' in _co):
                        if _i49excl: _heart_bundle=['협심증','빈맥','심부전']   # ★v367 지점장 확정 2026.08.06: 흥국(기타부정맥제외)=협심증·빈맥·심부전(허혈성 제거)
                        ...
                        elif _t==2: _heart_bundle=['협심증','염증']   # ★v367 지점장 확정 2026.08.06: 롯데 특정심장Ⅱ=협심증·염증(허혈성 제거)
            ...
            # ★★★★★v362 (지점장 지시 2026.08.06, 영구): 지점장 원문 —
            #   "<b>담보옆에 10년이라고 적힌건 갱신이다</b>".
            #   근거: KB 별첨은 셀 폭 때문에 `(10년갱신형)` 꼬리가 <b>물리적으로 잘려</b>
            #   `(10년갱` 에서 끝나는 담보가 있다(원본 PDF에 글자 자체가 없다 — 파서 무죄).
            #   구 규칙 `'갱신' in raw`는 이걸 못 잡아 <b>갱신형인데 검정</b>으로 찍혔다
            #   (실측 김민수 KB `표적항암방사선치료비(항암세기조절방사선)…(10년갱`).
            #   ★<b>실측 근거로 `N년갱`을 채택</b>: `\d+년`만으로 하면 삼성
            #     `[건강]종합병원 암 특정치료지원금(<b>진단 후 5년</b>, 연간1회한)` 등 3건이
            #     갱신으로 뒤바뀐다(그 계약은 [비갱신]이고 '5년'은 갱신주기가 아니라 보장기간).
            #     실측: `N년` 10건(오탐 3) / <b>`N년갱` 7건(오탐 0)</b> / 구 `갱신` 6건(누락 1).
            _gen10 = bool(re.search(r'\d+\s*년\s*갱', str(raw)))
            blue = gen or ('갱신' in raw) or _gen10      # ★ 담보명에 (갱신)·(N년갱) 표시 -> 파랑
            ...
            for tr in target_rows:
                ...
                _rep1 = std in ('표적항암치료비','다빈치로봇수술비','n대수술비','120대수술비','입원','통원','약값','약','간병인','간병인지원일당','창상봉합술','항암방사선약물','암수술','중입자치료비','암주요치료비','통합전이암','간호통합병동','합의금','6주미만','1인실 상급병원','1인실 종합병원')   # ★v302-B 6주미만=대표(max) / ★v198 합의금=대표1개 / ★v208 1인실 / ★v215 간병인지원일당=택일 대표(max)
                ...
                # ★★★★★v359 (지점장 지적 2026.08.06 "표적·양성자·세기가 (갱신)인데 다 까만색이다", 영구):
                #   한 마스터 행에 <b>담보가 여러 개 합쳐지면</b>(표적항암치료비 = 3대특정암 + 림프종 …)
                #   이 줄이 <b>담보마다 폰트를 통째로 덮어써서</b> 갱신 담보로 이미 파랑이 된 셀을
                #   뒤에 오는 비갱신(또는 갱신 글자가 잘린) 담보가 <b>검정으로 지웠다</b>.
                #   §10 정본 = `blue = gen OR ('갱신' in 담보명raw)` → <b>그 셀을 만든 담보 중 하나라도 갱신이면 파랑</b>이다.
                #   → <b>이미 파랑인 셀은 검정으로 되돌리지 않는다</b>.
                try:    _wasbl = str(getattr(getattr(ws.cell(tr,col).font,'color',None),'rgb','') or '').endswith('0070C0')
                except Exception: _wasbl = False
                ws.cell(tr,col).font = BL if (blue or _wasbl or std in ('입원','통원','약값','약','일상배상책임')) else BK
    ...
    if _sbc:
        for _ci2, _c2 in enumerate(contracts):
            ...
            _SEBU_WIN = {
                ...
                # ★★★★★v363 영구지침(지점장 지적 2026.08.06 "이제 아예 허혈성진단비를 안넣는다"):
                #   <b>'허혈성 진단비'를 이 집합에서 뺀다.</b>
                #   [원인] v359가 `_SEBU_LAB`·`_SEBU_PAIRS`의 허혈성 라벨을 None으로 바꿔 <b>보충</b>을 껐다
                #   (신정원 분류가 KB 특정Ⅰ 묶음까지 '허혈성심장질환진단비'로 뭉뚱그려 찍기 때문).
                #   그런데 이 `_SEBU_WIN` 삭제 루프는 그대로 남아, 세부값에 허혈성이 <b>영원히 없으므로</b>
                #   <b>별첨이 정상 기재한 값을 매번 지우기만</b> 했다 → 넣는 쪽만 끄고 지우는 쪽을 안 껐다.
                #   [실측] KB 별첨 30p `허혈성심장질환진단비(20년갱신형) 1,000` 기재 후 삭제(col=18,tr=50 로그 확인).
                #        현대해상 500도 같은 경로로 소실(동일 원인 — 미실측, 가설).
                #   ★정본(§8.3.1·KB전용): 허혈성 행의 판정 근거는 <b>별첨 회사담보명뿐</b>이다.
                #     세부가입현황·한장보장표는 신정원 분류표라 <b>넣지도 지우지도 않는다</b>.
                '통합전이암','뇌혈관진단비','뇌혈관수술비','뇌졸증진단비','심장수술비',
    ...
    for _ix3, _c3 in enumerate(contracts):
        ...
        for _nm3 in ('상해 종수술비(1-3종)', '질병 종수술비(1-3종)'):
            _r3 = nm2r.get(_nm3)
            if not _r3: continue
            # ★★★★★v365 (지점장 지시 2026.08.06, 영구): <b>1-3종은 강제 슬래시 표기</b>다.
            #   지점장 원문 = "<b>1-3종 강제 구현 1-3종 수술비 100 = 20/50/100 ← 질병·상해에 각각 강제값을 넣어라</b>".
            #   → 대표값(max) 숫자 하나가 아니라 <b>종별 슬래시 `50/100/200`</b>을 <b>질병·상해 두 행에 각각</b> 넣는다.
            #   ・끝열 합계는 기존 슬래시 합산 로직이 종별로 자동 처리한다(1~5종과 동일 방식).
            #   ・<b>[확인] 이 값은 금액과 무관한 고정 강제값이다</b> — 별첨 대표값이 200이 아닌 다른 금액일 때
            #     어떻게 할지는 지시에 없어 <b>내가 정하지 않는다</b>(제0조). 현재는 항상 `50/100/200`.
            ws.cell(_r3, _cl3).value = _SURG13_FORCE
            ws.cell(_r3, _cl3).font = BL if _gen3 else BK
            ws.cell(_r3, _cl3).alignment = Alignment(horizontal='center', vertical='center')
        print(f"[v365 1-3종 강제] {_cn3} {_c3.get('contract_date')} 대표 {_mx3:,.0f} → 질병·상해 종수술비(1-3종) = {_SURG13_FORCE}")
    ...
    # ══════════════════════════════════════════════════════════════════
    # ★★★★★v361 심장 배정 자가검증 게이트 (지점장 지적 2026.08.06
    #   "부정맥은 반복이고 지침과 다르게 생각대로 움직인다. 지침과메모리는 법률이다
    #    이걸 어기면 엑셀이 무너진다") — 영구, 절대 제거 금지
    #
    #   왜 필요한가: 부정맥 규칙은 v217에서 이미 정본으로 확정돼 코드에 들어 있었다.
    #   그런데 별첨 실물이 `부정맥질환(Ⅰ49)진단비`(로마숫자 Ⅰ)여서 조건에 <b>한 번도 매칭되지 않았다</b>.
    #   조문은 살아 있는데 <b>사문(死文)</b>이었고, 감사 케이스가 ASCII `I49`뿐이라 58/58 통과였다.
    #   → <b>규칙이 코드에 있는지</b>가 아니라 <b>실제 산출물이 조문을 지켰는지</b>를 매번 검사한다.
    #
    #   ★검사 대상은 전부 <b>기존 정본 조문</b>이다. 새 매핑 규칙을 만들지 않는다(제0조).
    #     (A) §8.3.1/KB전용: 기타심장부정맥(I49) = 부정맥 행 단독.
    #         원 담보명에 '부정맥'이 있고 시술어(고주파·절제·냉각·도자·시술)가 없는데
    #         부정맥 행이 비어 있으면 → 조문 위반 의심.
    #     (B) §8.3.1 공통원칙: '허혈성 진단비' 행 = 회사담보명 '허혈성심장질환진단비' 단독 담보 전용.
    #         묶음은 이 행에 절대 넣지 않는다. 허혈성 행에 값이 있는데 원 담보명에
    #         '허혈성'+'진단' 단독 담보가 하나도 없으면 → 조문 위반 의심.
    #     (C) 로마숫자+질병코드(Ⅰ49·Ⅰ47 등)가 원문에 남아 있으면 정규화 누락 신호 → 로그.
    #   ★값은 건드리지 않는다. 확인사항에 <b>시끄럽게</b> 띄우기만 한다(#11 영구원칙).
    # ══════════════════════════════════════════════════════════════════
    try:
        _hrows = {}          # 마스터 심장 행 → 합계
        for _r in range(2, ws.max_row+1):
            _nm = str(ws.cell(_r,2).value or '').strip()
            if _nm in ('부정맥','허혈성 진단비'):
                _s = 0
                for _c in range(3, last_col):
                    _v = ws.cell(_r,_c).value
                    if isinstance(_v,(int,float)): _s += _v
                _hrows[_nm] = _s
        _raws = [(t[0], str(t[1])) for t in heart_trace]          # (회사, 원담보명)
        _allraw = []
        for _ct in contracts:
            for _k in _ct.get('dambo',{}).keys():
                _allraw.append((_ct.get('company',''), str(_k)))
        # (A) 부정맥
        _brk = [(co,rw) for co,rw in _allraw
                if '부정맥' in re.sub(r'\s','',rw)
                and not any(x in rw for x in ('고주파','절제','냉각','도자','시술'))
                and '제외' not in rw]
        if _brk and not _hrows.get('부정맥',0):
            for co,rw in _brk[:3]:
                unmapped.append((0, co, '[정본위반] ' + rw[:60], 0,
                    '§8.3.1 기타심장부정맥(I49)=부정맥 행 단독인데 부정맥 행이 비었다 → 배정 확인'))
                print('[v361 HEART_AUDIT] (A) 부정맥 누락 | %s | %s' % (co, rw[:60]))
        # (B) 허혈성
        if _hrows.get('허혈성 진단비',0):
            _solo = [rw for co,rw in _allraw
                     if '허혈' in rw and '진단' in rw
                     and not any(x in re.sub(r'\s','',rw) for x in ('특정','심혈관','묶음','15대','순환계'))]
            if not _solo:
                unmapped.append((0, '—', '[정본위반] 허혈성 진단비 %s' % f"{_hrows['허혈성 진단비']:,}", 0,
                    "§8.3.1 '허혈성 진단비' 행은 회사담보명 '허혈성심장질환진단비' 단독 전용 · 묶음 금지 → 출처 확인"))
                print('[v361 HEART_AUDIT] (B) 허혈성 단독담보 없음인데 값 있음 | %s' % _hrows['허혈성 진단비'])
        # (C) 로마숫자 질병코드 잔존
        for co,rw in _allraw:
            if re.search(r'[ⅠⅡⅢⅣⅤ]\s*\d{2}', rw):
                print('[v361 HEART_AUDIT] (C) 로마숫자 질병코드 잔존 | %s | %s' % (co, rw[:60]))
    except Exception as _e:
        print('[v361 HEART_AUDIT ERR]', _e)
    ...
    try:
        ...
        if _hj:
            ...
            _pairs = [
                ...
                ('뇌혈관수술비',      _g('뇌혈관수술비')),
                # ★v362 심장 3항목 제외(아래 주석) — 허혈성 진단비 · 급성심근경색
                ...
                # ★v362 심장 3항목 제외 — 심장수술비
                # ══════════════════════════════════════════════════════════
                # ★★★★★v362 (지점장 지시 2026.08.06, 영구): 지점장 원문 —
                #   "<b>심장질환은 1장표를 볼것이아니라 각 가입한 보험별로 봐야한다</b>".
                #   근거: 한장보장표는 <b>신정원 분류표</b>다. 신정원은 KB 특정Ⅰ 같은
                #   <b>묶음 담보도 '허혈성심장질환진단비'로 뭉뚱그려</b> 찍는다.
                #   BARUM 정본(§8.3.1)은 '허혈성 진단비' 행 = 회사담보명 단독 담보 전용이므로
                #   한장표와 구조적으로 어긋나는 것이 <b>정상</b>이다(교통상해사망 선례와 동일).
                #   실측(김민수 KB): 별첨에 허혈성 단독 담보 없음 · 한장표 1,000 · 엑셀 0 → 정상.
                #   → 심장질환 3항목(<b>허혈성 진단비 · 급성심근경색 · 심장수술비</b>)은
                #     한장표 검산 대상에서 제외한다. <b>대신 계약별 별첨 근거표(심장 블록)로 본다.</b>
                #   ★조용히 끄지 않는다 — 왜 뺐는지 확인사항 [검산] 표 아래에 남긴다.
                # ══════════════════════════════════════════════════════════
                ('상해수술비',        _g('상해수술비')),
                ('질병수술비',        _g('질병수술비')),
                ('상해일당',          _g('상해일당')),
            ...
            # ★★★★★v362 (지점장 지시 2026.08.06): 심장 3항목 검산 제외 사유를 <b>반드시 남긴다</b>.
            #   조용히 끄면 아무도 모른다(#11 「조용히 틀리는 것을 시끄럽게 틀리는 것으로 바꾼다」).
            _rr += 2
            ws2.cell(_rr,1,'[검산 제외] 심장질환 3항목 — 한장보장표로 보지 않는다 (지점장 지시 2026.08.06)')
            ws2.cell(_rr,1).font = Font(bold=True, size=11, color='7A5C00')
            _rr += 1
            ws2.cell(_rr,1,'허혈성 진단비 · 급성심근경색 · 심장수술비')
            ws2.cell(_rr,2,'한장보장표=신정원 분류표라 묶음담보를 뭉뚱그린다 → 계약별 별첨(아래 심장 근거표)으로 대조할 것')
            if _exl:
...
def build_ppt(data, out, totals=None, surg_q=None, surg_s=None, splits=None):
    ...
    def pvl(box, label, std):
        ...
        if isinstance(_T,(int,float)) and _T>0 and (gs+ns)!=_T:
            if   gs and not ns: gs,ns=int(_T),0
                ...
                # ★★★★★v365: <b>gs+ns가 0이면 나눌 수 없다</b>. 엑셀에 직접 기재되는 담보
                #   (1-3종 종수술비 등)는 dambo를 거치지 않아 갱신/비갱신 분할값이 둘 다 0이다.
                #   구 코드는 그대로 나눠 <b>ZeroDivisionError로 PPT 생성이 통째로 죽었다</b>(실측 이영태).
                #   → 분할 근거가 없으면 <b>비갱신(검정)으로 전액</b> 표기하고 넘어간다(엑셀 셀 색이 원천이나
                #     이 경로엔 색 정보가 오지 않는다 — 값 소실보다 검정 표기가 안전하다).
                _tt=gs+ns
                if _tt<=0: gs,ns=0,int(_T)
                else: gs=int(round(_T*gs/_tt)); ns=int(_T)-gs
        if not gs and not ns: return
    ...
    # ★★★★★v363 (지점장 지적 2026.08.06 "1-3종 수술비 안 나온다", 영구):
    #   폼 `TextBox 17`(질병)·`TextBox 19`(상해) <b>p2에 `1~3종수술비 :` 라벨이 이미 있는데</b>
    #   <b>주입 코드가 아예 없었다</b> — 1~7종·1~5종만 채우고 1~3종은 한 번도 쓰지 않았다.
    #   [실측] 엑셀 상해 69행·질병 77행 <b>둘 다 200</b>인데 분석지 PPT는 <b>두 칸 모두 빈칸</b>
    #        = 결과값 동결 위반(엑셀 ≠ 보장분석지 PPT).
    #   ★암주요치료비(v360)·급성심근(v363)과 같은 유형 — <b>폼에 칸은 있는데 코드가 조회조차 안 하는</b> 무언실패.
    # ★★★★★v365 (지점장 지시 2026.08.06, 영구): <b>1-3종은 강제 슬래시 `20/50/100`</b>을
    #   엑셀·PPT에 <b>같은 문자열</b>로 찍는다(§1 결과값 동결 — 넷이 달라질 수 없다).
    #   ★`pvl`은 갱신/비갱신 숫자 분할용이라 슬래시를 못 넣는다 → 라벨 슬롯에 <b>문자열을 직접</b> 기입한다.
    def _pv13(box, label, txt):
        sl=_slot(box,label)
        if not sl:
            print(f'[PPT_MISS] 라벨없음 box={box} label={label!r}'); return
        p, ri, c = sl
        t=p.runs[ri].text; head=t[:c+1]; tail=t[c+1:]
        if tail.strip() and not re.search(r'[^\d,/\s]', tail): tail=''
        _seg(p.runs[ri], [(f'{head} {txt}',_BLACK),(tail,None)])
    if g('상해 종수술비(1-3종)'): _pv13('TextBox 19','1~3종수술비',_SURG13_FORCE)
    if g('질병 종수술비(1-3종)'): _pv13('TextBox 17','1~3종수술비',_SURG13_FORCE)
    ...
    # ★★★★★v360 (지점장 지적 2026.08.06 "종합병원암주요치료비 10,000 암주요치료비에 넣어야하는데 지침무시다"):
    #   폼 `TextBox 57`에는 <b>`암주요치료비 :` 라벨이 있는데 값을 채우는 코드가 아예 없었다</b>.
    #   하이클래스·10억통장 두 줄만 주입하고 <b>첫 줄을 빠뜨려</b> 엑셀에 10,000이 있어도 PPT는 항상 빈칸이었다.
    #   등식2(4대 산출물 동결) 위반 — 엑셀 = 보장분석지 PPT 는 어긋날 수 없다.
    #   실측(김민수 삼성화재): 별첨 `[건강]종합병원 암 특정치료지원금(진단 후 5년, 연간1회한)` 10,000
    #     → 엑셀 암주요치료비 10,000 ○ / 보장분석지 PPT 빈칸 ✗ → 수정 후 `암주요치료비 : 1억` ○
    ...
    if g('간병인'): pvl('TextBox 22','간병인일당','간병인')
...
def health():
    ...
    return {'ok':True,'version':'v368-simjang-20260806',
            'audit': _audit,
...
def version_bot():
    ...
    out = {'server_version': 'v368-simjang-20260806'}
...
def doctrine_bot():
    ...
    try:
        ...
        cov['검사'] = 'ERR ' + str(e)[:40]
    return {'version': 'v368-simjang-20260806',
            '지침정본': _DOCTRINE_SRC,
...
def diag():
    ...
    out = {'version': 'v368-simjang-20260806'}
